    ...
    <section className="relative min-h-screen flex flex-col items-center justify-center bg-[#fafbfc] overflow-hidden px-4 font-sans gap-2 py-6">
      ...
      <div className="flex items-center justify-center mb-4">
        <div className="flex items-center gap-3">
          ...
          <Link to="/" className="text-xl font-semibold text-gray-900 hover:text-blue-400 transition-colors">
            SOLEASE
          </Link>
        </div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-[440px] bg-white/40 backdrop-blur-xl px-8 md:px-10 py-7 rounded-[32px] border border-gray-300/5 shadow-2xl">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="flex justify-between gap-3 mb-6">
            {code.map((digit, index) => (
              <input
                ...
                className="w-14 h-14 text-center text-lg font-semibold bg-gray-100 border-2 border-gray-300/5 rounded-xl text-gray-900 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 outline-none transition-all"
              />
            ))}
          </div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="w-full py-4 bg-blue-500 hover:bg-blue-400 text-[#0a0a0a] rounded-xl font-medium shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2 group uppercase tracking-wider"
          >
            {isLoading ? (
