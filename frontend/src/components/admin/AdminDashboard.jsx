...
import { Plus, Search, Tickets, Users, CheckCircle, Clock, AlertTriangle, TrendingUp, ArrowRight, BarChart3, List, Zap, Calendar, X, FileText, MapPin, Bot } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const quickActions = [
    ...
    { label: "AI Assistant", icon: Bot, path: "/admin-dashboard/ai-chat", color: "orange" },
    { label: "Pending Tickets", icon: AlertTriangle, path: "/admin-dashboard/admin-pending-tickets", color: "purple" },
  ];
    ...
    <DashboardLayout>
      ...
      >
            ...
            {quickActions.map((link, i) => (
              ...
              >
                <div
                  ...
                  className="flex flex-col p-5 rounded-2xl bg-white dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700/50 shadow-sm hover:shadow-xl hover:border-blue-200 dark:hover:border-blue-700 transition-all duration-300 h-full relative overflow-hidden cursor-pointer"
                >
                  <div className={`absolute top-0 right-0 w-24 h-24 rounded-full -mr-8 -mt-8 opacity-10 transition-transform duration-300 group-hover:scale-150 ${link.color === 'blue' ? 'bg-blue-500' : link.color === 'indigo' ? 'bg-indigo-500' : link.color === 'purple' ? 'bg-purple-500' : link.color === 'orange' ? 'bg-orange-500' : 'bg-violet-500'}`} />
                  <div className={`inline-flex items-center justify-center w-12 h-12 rounded-xl mb-4 shadow-lg ${link.color === 'blue' ? 'bg-gradient-to-br from-blue-500 to-blue-600 shadow-blue-500/30' : link.color === 'indigo' ? 'bg-gradient-to-br from-indigo-500 to-indigo-600 shadow-indigo-500/30' : link.color === 'purple' ? 'bg-gradient-to-br from-purple-500 to-purple-600 shadow-purple-500/30' : link.color === 'orange' ? 'bg-gradient-to-br from-orange-500 to-orange-600 shadow-orange-500/30' : 'bg-gradient-to-br from-violet-500 to-violet-600 shadow-violet-500/30'}`}>
                    <link.icon size={22} className="text-white" />
                  </div>
                  <h3 className="text-sm font-medium text-gray-800 dark:text-gray-100 mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                    {link.label}
        ...
        <motion.div
          ...
          >
            <div className="p-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-800/50">
              <h4 className="text-xs font-medium text-gray-700 dark:text-gray-200 flex items-center gap-2">
                <Bot size={18} className="text-purple-600 dark:text-purple-400" />
                AI Assistant Chats
              </h4>
              <button
                onClick={() => navigate("/admin-dashboard/ai-chat-history")}
            ...
            {aiSessions.length > 0 ? (
              <div className="p-5 space-y-3">
                {aiSessions.map((session) => (
                  <div
                    ...
                    onClick={() => navigate(`/admin-dashboard/ai-chat/${session._id || session.id}`)}
                    className="flex items-center gap-3 p-3 rounded-xl bg-purple-50 dark:bg-purple-900/20 hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors cursor-pointer"
                  >
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-4">
                  Start chatting with our AI assistant to get instant help with ticket management.
                </p>
                <button
