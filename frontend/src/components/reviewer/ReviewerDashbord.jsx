  ...
  const displayTickets = assignedTickets
    ...
    .sort((a, b) => {
      ...
      if (sortConfig.key === 'urgency') {
        ...
        return sortConfig.direction === 'asc' ? aUrgency - bUrgency : bUrgency - aUrgency
      }
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          >
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">
              {showWelcome
                ? "Start by reviewing available tickets or create a new one."
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                >
                  ...
                  >
                    <div className={`absolute top-0 right-0 w-24 h-24 rounded-full -mr-8 -mt-8 opacity-10 transition-transform duration-300 group-hover:scale-150 ${link.color === 'blue' ? 'bg-blue-500' : link.color === 'indigo' ? 'bg-indigo-500' : link.color === 'orange' ? 'bg-orange-500' : 'bg-emerald-500'}`} />
              ...
              <div className="overflow-x-auto">
                <table className="w-full table-auto border-collapse">
                  <thead className="bg-gray-50/50 dark:bg-gray-800/50">
                    <tr>
                      {["Ticket", "Subject", "Type", "Urgency", "Status", "Client", "Location", "Updated", "Attachments", "Feedback", ""].map((header) => {
                        const sortKey = header === "Ticket" ? "ticketId" : header === "Urgency" ? "urgency" : header === "Updated" ? "updatedAt" : null;
                        return (
                          <th
                            key={header}
                            className={`px-4 py-3 text-[10px] text-left font-bold text-gray-400 uppercase tracking-widest ${
                              sortKey ? "cursor-pointer hover:text-gray-600 dark:hover:text-gray-300" : ""
                            } ${header === 'Attachments' || header === 'Feedback' || header === 'Location' ? 'hidden md:table-cell' : ''}`}
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <motion.span
                              ...
                              className="inline-block font-mono text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/40 px-2 py-1 rounded-md"
                            >
                        ...
                        <td className="px-4 py-3">
                          <motion.span
                            ...
                            animate={{ scale: 1 }}
                            whileHover={{ scale: 1.05 }}
                            className={`inline-block px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all duration-200 ${
                              ticket.urgency === "Critical"
                        ...
                        <td className="px-4 py-3 text-center">
                          {ticket.comments && ticket.comments.length > 0 ? (
                            ...
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-400 rounded-md text-xs">
                              0
                            </span>
                          )}
                        </td>
                    ...
                    {displayTickets.map((ticket) => (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-3">
                          <div className="flex items-center gap-1">
                            <div className="w-5 h-5 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white text-[8px] font-bold">
                              {ticket.user?.name?.charAt(0) || ticket.user?.username?.charAt(0) || '?'}
                            </div>
                            <span className="truncate max-w-[80px]">{ticket.user?.name || ticket.user?.username || 'N/A'}</span>
                          </div>
                  ...
                  return hasMessages ? (
                            ...
                            <div key={role} className="space-y-3">
                              ...
                              <div className="space-y-3">
                                {roleGroups[role].map((msg, idx) => (
                                  <Link
                                    key={`${role}-${idx}`}
                                    to={`/reviewer-dashboard/ticket/${msg.ticketId}/feedback`}
                    ...
                    <div className="py-6 text-center">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto">
                        Client feedback and responses will appear here once tickets are reviewed.
                      </p>
                    </div>
                  )
                })()}
